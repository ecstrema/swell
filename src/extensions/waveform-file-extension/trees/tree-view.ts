...
export class TreeView extends HTMLElement {
    ...
    constructor(settingsExtension: SettingsExtension) {
        ...
        this.settingsExtension.onSettingChange(TreeView.INDENT_SETTING_PATH,  (value) => this.updateIndent(value as number));
    ...
    private async loadIndentSetting() {
        try {
            const indent = await this.settingsExtension.getValue<number>(TreeView.INDENT_SETTING_PATH);
            if (indent !== undefined) {
